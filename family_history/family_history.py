...
def main():
    ...
    # Call the count_marriages function to count
    # and print the number of marriages for each person
    # in person_dict dictionary
...
def print_death_age(people_dict):
    ...
    for key, value in people_dict.items():
        ...
        print(f"Name: {name}, Birth year: {dob}, Death year: {dod} Age at death: {age_at_death}")       
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key, value in marriages_dict.items():
        ...
        husband_age_at_wedding = marriage_year - husband_birth_year
        wife_age_at_wedding = marriage_year - wife_birth_year
        print(f"{husband_name} {husband_age_at_wedding} > {marriage_year} < {wife_name} {wife_age_at_wedding}")

def count_marriages(marriage_dict, people_dict):
    marriage_count = {} # Initialize an empty dictionary to store marriage counts for each person
    for key, value in marriage_dict.items(): # Iterate through the marriages dictionary
        husband_key = value[HUSBAND_KEY_INDEX]
        wife_key = value[WIFE_KEY_INDEX]
        husband_data = people_dict[husband_key]
        wife_data = people_dict[wife_key]
        husband_name = husband_data[NAME_INDEX]
        wife_name = wife_data[NAME_INDEX]
        if husband_key in people_dict: # Check if the husband and wife are in the people dictionary
            husband_name = people_dict[husband_key][NAME_INDEX] # Update marriage count for the husband
            if husband_name in marriage_count:
                marriage_count[husband_name] += 1
            else:
                marriage_count[husband_name] = 1
        if wife_key in people_dict:
            wife_name = people_dict[wife_key][NAME_INDEX] # Update marriage count for the wife
            if wife_name in marriage_count:
                marriage_count[wife_name] += 1
            else:
                marriage_count[wife_name] = 1

    for person_name, count in marriage_count.items(): # Prints the count of marriages for each person
        print(f"{person_name} had {count} marriage(s).")
